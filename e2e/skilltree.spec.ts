...
test("veterans are seeded their old skills plus retroactive points", async ({ page }) => {
  ...
  await page.locator(".skill-node", { hasText: "Iron Skin" }).getByRole("button", { name: "Learn (1 pt)" }).click();
...
test("upgrades patch the battle skill they improve", async ({ page }) => {
  ...
  await page.locator(".skill-node", { hasText: "Blood Rage" }).getByRole("button", { name: "Learn (1 pt)" }).click();
  await page.getByRole("button", { name: "Close" }).click();
